...
export default {
  ...
  methods: {
    ...
    getExamData() {
      ...
      let examData = this.$store.state.examData; //获取路由传递过来的试卷信息
      this.examData = this.$store.state.examData; //获取路由传递过来的试卷信息
      this.index = 0;
      this.time = examData.totalTime; //获取分钟数
      
      let paperId = examData.paperId;
      this.examData.type=examData.type
      this.examData.source=examData.source
      request.get(`/api/paper/${paperId}`).then((res) => {
        //通过paperId获取试题题目信息
        this.topic = res;
        console.log(res);
          ...
          .then(() => {
            ...
            request.post(
              "/api/insertScore",
              {
                ...
                studentId:this.$store.state.userInfo.user, //学号
