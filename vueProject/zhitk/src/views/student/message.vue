...
export default {
  ...
  data() {
    return {
      ...
      form:{
        title: this.title,     // 确保 this.title 已经在 Vue 实例中定义
        content: this.content, // 确保 this.content 已经在 Vue 实例中定义
        time: this.date        // 确保 this.date 已经在 Vue 实例中定义
  ...
  methods: {
    getMsg() {
      ...
      request
        .post("/api/selectMessage",searchPageDto)
        .then((res) => {
